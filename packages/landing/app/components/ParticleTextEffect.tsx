...
export function ParticleTextEffect({ words = DEFAULT_WORDS, className = "" }: ParticleTextEffectProps) {
  ...
  const nextWord = useCallback((word: string, canvas: HTMLCanvasElement) => {
    ...
    offscreenCtx.font = "bold 80px 'Inter', Arial, sans-serif"
    ...
    // Generate new color - use 1shot brand colors
    const brandColors = [
  ...
  return (
    <div className={`flex flex-col items-center justify-center ${className}`}>
      <canvas
        ref={canvasRef}
        className="border border-gray-800 rounded-lg shadow-2xl bg-black"
        style={{ maxWidth: "100%", height: "auto" }}
      />
      <div className="mt-4 text-gray-600 dark:text-gray-300 text-sm text-center max-w-md">
        <p className="mb-2">Interactive Particle Effect</p>
