...
export class Cursor {
  ...
  public getOffsetFromCoordinates(x: number, y: number): number {
    return this.measuredText.getOffsetFromCoordinates(x, y)
  }
}
...
export class MeasuredText {
  ...
  public getOffsetFromCoordinates(x: number, y: number): number {
    ...
    const line = Math.max(0, Math.min(y, this.wrappedLines.length - 1))
    const wrappedLine = this.getLine(line)
    
    // Account for leading whitespace that was trimmed in display
    ...
    const maxColumn = wrappedLine.text.length - leadingWhitespace
    const column = Math.max(0, Math.min(x, maxColumn))
    
    return this.getOffsetFromPosition({ line, column })
  }
}